...
class UI:
    ...
    def read_graph_from_given_file(self):
        ...
        try:
            self.graph = read_activities(input_file)
            print("done!")
        except Exception as e:
            print(e)
    ...
    # the eighteenth option
    def topological_sorting(self):
        sorted_vertices = self.graph.toposort()
        if (sorted_vertices == False):
            print("the graph is not a DAG!")
        else:
            print("the topological sorting of the graph is: " + str(sorted_vertices))
    
    # the nineteenth option
    def earliest_and_latest_times(self):
        earliest_start, earliest_end, latest_start, latest_end, duration = self.graph.get_times()
        if duration == -1:
            print("the graph is not a DAG!")
        else:
            ...
            for i in range(self.graph.number_of_vertices()):
                table_list.append([str(i), str(earliest_start[i]) + "-" + str(earliest_end[i]), str(latest_start[i]) + "-" + str(latest_end[i])])

            t.add_rows(table_list)
            print(t.draw())
            print("the duration of the project is: " + str(duration))

    # the twentieth option
    def critical_activities(self):
        critical = self.graph.critical_activities()
        if not critical:
            print("the graph is not a DAG!")
        else:
    ...
    def print_menu():
        ...
        print("19. earliest and latest times.")
        print("20. critical activities.")
        print("0. exit")
    ...
    def start(self):
        options = { "1": self.random_graph_with_given_number_of_vertices_and_edges,
                    ...
                    "18": self.topological_sorting,
                    "19": self.earliest_and_latest_times,
                    "20": self.critical_activities}
        while True:
            self.print_menu()
            command = input("> ")
