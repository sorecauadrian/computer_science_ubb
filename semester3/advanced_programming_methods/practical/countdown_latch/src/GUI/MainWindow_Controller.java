...
public class MainWindow_Controller implements Initializable
{
    ...
    //new additions for count down latch
    @FXML
    private TableView<Map.Entry<Integer,Integer>> latch_table_view;
    @FXML
    private TableColumn<Map.Entry<Integer,Integer>,Integer> latch_table_addresses;
    @FXML
    private TableColumn<Map.Entry<Integer,Integer>,String> latch_table_values;
//
    ...
    //new additions for count down latch
        latch_table_addresses.setCellValueFactory(p->new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        latch_table_values.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue() + " "));
    //
    ...
    private void change_program_state_handler(ProgramState current_program_state)
    {
        ...
        try
        {
            ...
            populate_latch_table_view(current_program_state);
        }
        catch (Exception e)
        {
    ...
    private void populate_latch_table_view(ProgramState program)
    {
        latch_table_view.setItems(FXCollections.observableList(new ArrayList<>(program.get_latch_table().get_content().entrySet())));
