...
public class SelectWindow_Controller  implements Initializable
{
    ...
    private List<Statement> initialise_programs()
    {
        ...
        Statement first_fork = new ForkStatement(new CompoundStatement(new AwaitStatement(new VariableExpression("cnt")),
                new CompoundStatement(new Heap_WriteStatement("v1", new ArithmeticExpression(new Heap_ReadExpression(new VariableExpression("v1")), new ValueExpression(new IntValue(10)),"*")),
                        new PrintStatement(new Heap_ReadExpression(new VariableExpression("v1"))))));
        Statement second_fork = new ForkStatement(new CompoundStatement(new AwaitStatement(new VariableExpression("cnt")),
                new CompoundStatement(new Heap_WriteStatement("v2", new ArithmeticExpression(new Heap_ReadExpression(new VariableExpression("v2")), new ValueExpression(new IntValue(10)),"*")),
                        new CompoundStatement(new Heap_WriteStatement("v2", new ArithmeticExpression(new Heap_ReadExpression(new VariableExpression("v2")), new ValueExpression(new IntValue(10)),"*")),
                                new PrintStatement(new Heap_ReadExpression(new VariableExpression("v2")))))));
        Statement statement_for_barrier= new CompoundStatement(new Variable_DeclarationStatement("v1", new ReferenceType(new IntType())),
                                        ...
                                        new CompoundStatement(new NewStatement("v1",new ValueExpression(new IntValue(2))),
                                                new CompoundStatement(new NewStatement("v2",new ValueExpression(new IntValue(3))),
                                                        new CompoundStatement(new NewStatement("v3",new ValueExpression(new IntValue(4))),
                                                                new CompoundStatement(new New_BarrierStatement("cnt", new Heap_ReadExpression(new VariableExpression("v2"))),
                                                                        new CompoundStatement(first_fork,
                                                                                new CompoundStatement(second_fork,
                                                                                        new CompoundStatement(new AwaitStatement(new VariableExpression("cnt")),
                                                                                                new PrintStatement(new Heap_ReadExpression(new VariableExpression("v3"))))))))))))));

        statements_list.add(statement_for_barrier);
