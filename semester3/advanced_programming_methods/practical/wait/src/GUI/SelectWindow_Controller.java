...
public class SelectWindow_Controller  implements Initializable
{
    ...
    private List<Statement> initialise_programs()
    {
        ...
        Statement wait_statement=new CompoundStatement(new Variable_DeclarationStatement("v",new IntType()),
                new CompoundStatement(new AssignmentStatement("v",new ValueExpression(new IntValue(20))),
                new CompoundStatement(new WaitStatement(10),new PrintStatement(new ArithmeticExpression(new VariableExpression("v"),new ValueExpression(new IntValue(10)),"*")))));

        statements_list.add(wait_statement);
