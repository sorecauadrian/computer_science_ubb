...
public class MainWindow_Controller implements Initializable
{
    ...
    //new additions for lock table
    @FXML
    private TableView<Map.Entry<Integer,Integer>> lock_table_view;
    @FXML
    private TableColumn<Map.Entry<Integer,Integer>,Integer> lock_table_addresses;
    @FXML
    private TableColumn<Map.Entry<Integer,Integer>,String> lock_table_values;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
        ...
        //new addition for lock
        lock_table_addresses.setCellValueFactory(p->new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        lock_table_values.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue() + " "));
    ...
    private void change_program_state_handler(ProgramState current_program_state)
    {
        ...
        try
        {
            ...
            populate_lock_table_view(current_program_state);
    ...
    private void populate_lock_table_view(ProgramState program)
    {
        lock_table_view.setItems(FXCollections.observableList(new ArrayList<>(program.get_lock_table().get_content().entrySet())));
