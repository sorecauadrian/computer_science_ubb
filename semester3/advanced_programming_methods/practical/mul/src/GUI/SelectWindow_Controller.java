...
public class SelectWindow_Controller  implements Initializable
{
    ...
    private List<Statement> initialise_programs()
    {
        ...
        Statement mul_statement=new CompoundStatement(new Variable_DeclarationStatement("v1",new IntType()),
                new CompoundStatement(new AssignmentStatement("v1",new ValueExpression( new IntValue(2))),
                        new CompoundStatement(new Variable_DeclarationStatement("v2",new IntType()),
                                new CompoundStatement(new AssignmentStatement("v2",new ValueExpression(new IntValue(3))),
                                        new IfStatement(new RelationalExpression(new VariableExpression("v1"),new ValueExpression(new IntValue(0)),"!="),new PrintStatement(new MULExpression(new VariableExpression("v1"),new VariableExpression("v2"))),new PrintStatement(new VariableExpression("v1")))))));


        statements_list.add(mul_statement);
