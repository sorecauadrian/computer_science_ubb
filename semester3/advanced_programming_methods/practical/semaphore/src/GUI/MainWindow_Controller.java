...
public class MainWindow_Controller implements Initializable
{
    ...
    //new additions for semaphore
    @FXML
    private TableView<Map.Entry<Integer, Pair<Integer, List<Integer>>>> semaphore_table_view;
    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphore_table_indexes;
    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphore_table_values;
    @FXML
    private TableColumn<Map.Entry<Integer, Pair<Integer, List<Integer>>>, String> semaphore_table_list;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle)
    {
        ...
        semaphore_table_indexes.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getKey() + " "));
        semaphore_table_list.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue().getValue() + " "));
        semaphore_table_values.setCellValueFactory(p->new SimpleStringProperty(p.getValue().getValue().getKey() + " "));
    ...
    private void change_program_state_handler(ProgramState current_program_state)
    {
        ...
        try
        {
            ...
            populate_semaphore_table_view(current_program_state);
        }
        catch (Exception e)
        {
    ...
    public void execute_one_step_handler(ActionEvent actionEvent)
    {
        ...
        try
        {
            ...
            if(controller.get_repository().get_program_states_list().size()==0)
            {
                execute_button.setDisable(true);
            }
        }
    ...
    private void populate_semaphore_table_view(ProgramState program)
    {
        semaphore_table_view.setItems(FXCollections.observableList(new ArrayList<>(program.get_semaphore_table().get_semaphore_table().entrySet())));
    ...
    private ProgramState get_selected_program()
    {
        ...
        {
            Alert error=new Alert(Alert.AlertType.ERROR,"No Thread Selected!");    //needs more testing, change to return null if it breaks
            error.show();
            return null;
        }

        int id=program_id_view.getSelectionModel().getSelectedItem();
